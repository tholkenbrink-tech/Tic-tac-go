...
export function expectedPiece(turn: number): PieceId {
  const piece = PIECE_ORDER[((turn % 6) + 6) % 6]
  if (!piece) throw new Error('unreachable')
  return piece
}

/** Piece that acts after the given turn. */
export function followingPiece(turn: number): PieceId {
  return expectedPiece(turn + 1)
}
...
export function isMoveValid(
  ...
  if (piece !== expectedPiece(turn)) return false
...
export function validDestinations(board: Board, turn: number, piece: PieceId): CellIndex[] {
  if (phaseForTurn(turn) !== 'movement' || piece !== expectedPiece(turn)) return []
  const out: CellIndex[] = []
  for (let i = 0; i < 9; i++) {
